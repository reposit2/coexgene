def load_ml_data2(deg_data_file,
                 promoter_data_loc,
    ...
    return deg_data, promoter_data, promoter_feature_name

def prep_ml_data_split4(deg_data_file,
                       promoter_data_loc,
                       promoter_annotation_data,
                       train_genes,
                       validate_genes,
    ...
    deg_data, promoter_data, promoter_feature_name=load_ml_data2(deg_data_file,
                                                                                                         promoter_data_loc,
                                                                                                         promoter_annotation_data)
    ...
    DEG_scale_stats = pd.DataFrame({'feature_name' : np.array(Y_train.columns)[1:(Y_train.shape[1]-1)],
                                    'row_indx': list(range(0,Y_train.shape[1]-2)),
    ...
    promoter_feature_norm_stats = pd.DataFrame({'feature_name' : promoter_feature_name,
                                                'row_indx': list(range(len(promoter_feature_name))),
    ...
    return Y_train, Y_validate, Y_test, X_promoter_train, X_promoter_validate, X_promoter_test

def batch_iter2(X_promoter_train,
               Y_train,
               batch_size,
               shuffle=True):
...
#    num_batches_per_epoch2 = np.asarray(num_batches_per_epoch, dtype=np.float32)
...
#                seq_len_mRNA=[np.shape(x)[1] for x in shuffled_data_mRNA[start_index: end_index]]
