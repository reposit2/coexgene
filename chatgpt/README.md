...
0. Data Preperation (this step can be omitted for testing purposes)
   ...
   If need to download GO, follow the code below: 
    ...
    ```
    and the [notebook](code/0.[Prep%20GO]Download_and_parse_GO.ipynb) for parsing GO terms
	
    The addition of contamination to the gene set is filed in [this notebook](code/0.%20[GO%20set]add_random_contamination.ipynb)

	If need to download Omics data, run [notebook](code/0.[Omics_revamped]_ProcessOmicsData.ipynb). The notebook processes the omics data and saves them into a tab delimited text file.
   

1. Query GPT-4 for names and supporting analysis and run functional enrichment

   GO gene set GPT-4 analysis is stored in [Run_LLM_analysis](code/1.[GO%20set]Run_LLM_analysis.ipynb)
 
   GO gene set analysis with [different models](code/1A.[GO%20set]Compare_models.ipynb)

   Batch run 1000 GO terms using [slurm job](code/thousandGOsets_GPT4Run.sh) with the [parameter file](thousandGOsets_GPT4Run_params.txt) 


    [omic gene set GPT-4 analysis](code/1A.[Omics_revamped]GenerateLLM_analysis.ipynb) and [omics gene set gProfiler](code/1B-2.[Omics_revamped]run_gProfiler.ipynb)
...
2. Semantic Similarity evaluation of names
    ...
    [GO gene set analysis evalution](code/2.[GO%20set]Rank_LLM_GO_term_pair_sim.ipynb)
...
3. Further evaluation of the performance: model comparison evaluation, gene set functional enrichment, and gene set similarity comparison
    ...
    *Model Comparison*
    ...
    [Compare the semantic similarities between models](code/3A.[model%20compare]compare_semantic_similarity.ipynb)

   Analysis related to Fig. 3
    [Run GO gene set functional enrichment for control](code/3A.[model%20compare]functional_enrichment_analysis_control.ipynb)

   [Compare the confidence score between real, contaminated, and random gene sets](code/3B-2.[model%20compare]Check_confidence_scoring_metrics.ipynb)

    *Check broader concepts of the LLM names*
   
    Analysis for Fig. 2d
   
    [Analysis for whether the best matching GO term is a broader concept as the queried term](code/3C.[GO%20set]Evaluate_gene_set_similarity.ipynb)

    **Evaluation Task 2 related**
    [Count genes supporting LLM name](code/2A.[Omics_revamped]_CountSupportingGenes.ipynb), then [calculate LLM name Jaccard Index](code/2B.[Omics_revampled]Calculate_LLM_JI.ipynb)

    Analysis related to Fig.4 

    [Omics data naming evaluation](code/3A.[omics_revamped]Analyze_gprofiler_annotation.ipynb)

    Evaluate LLM name matching with any significantly enriched GO term name, use [this notebook](code/3B[Omics_revamped]Compare_enriched_GO_and_LLM_name_sim.ipynb)




6. Development and assessment of the [citation module](code/4.Reference%20search%20and%20validation.ipynb)


7. Quantification of citation module [check citation module](code/5.Quantify%20reference%20checking.ipynb)

8. Visualization of results

    [extended data fig.1 + Fig.2 + Fig.3](code/6.[GO%20set]Plot_GO_analysis_figs.ipynb)
   
    [extract sub hierarchy (Fig.2e)](code/6.[GO%20set]%20subhierarchy_GO_example.ipynb)
   
    [Omics figures (Fig 4, Extended Data Fig.5)](code/6B.[Omics_revamped]GenerateOmicsFigures.ipynb)
